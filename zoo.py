...
class VGGTModel(fout.TorchImageModel, fout.TorchSamplesMixin):
    ...
    def _build_transforms(self, config):
        """Build minimal transforms for VGGT preprocessing.
        
        VGGT requires specific preprocessing (518px target size, crop mode) which
        is handled separately in _preprocess_vggt_image(). This method provides
        only basic PIL conversion for compatibility with FiftyOne's pipeline.
        
        Args:
            config: Model configuration (unused for VGGT)
            
        Returns:
            tuple: (transform_composition, ragged_batches_flag)
                - transform_composition: Minimal torchvision transforms
                - ragged_batches_flag: True, indicating variable-size processing
        """
        # VGGT has custom preprocessing, so we only need basic PIL conversion
    ...
    def _preprocess_vggt_image(self, img):
        """Preprocess PIL image according to VGGT's specific requirements.
        
        VGGT requires images to be:
        1. 518 pixels wide (target_size)
        2. Height scaled proportionally and divisible by 14 (for transformer patches)
        3. Center-cropped if height exceeds 518 pixels
        4. Converted to tensor format [C, H, W] in range [0, 1]
        
        Args:
            img (PIL.Image): Input image from FiftyOne dataset
            
        Returns:
            tuple: (preprocessed_tensor, original_size)
                - preprocessed_tensor: Torch tensor [C, H, W] ready for VGGT
                - original_size: (width, height) of original image for coordinate conversion
        """
        # Store original dimensions for coordinate transformations
        original_size = img.size  # (width, height)
        
        # VGGT preprocessing parameters
        target_size = 518  # Standard VGGT input width
        mode = "crop"      # VGGT's default preprocessing mode
        ...
        if mode == "pad":
            # Pad mode: maintain aspect ratio, pad to square (alternative approach)
            if width >= height:
                new_width = target_size
                new_height = round(height * (new_width / width) / 14) * 14  # Divisible by 14
            else:
                new_height = target_size
                new_width = round(width * (new_height / height) / 14) * 14   # Divisible by 14
        else:  # mode == "crop" (VGGT default)
            # Crop mode: set width to 518px, scale height proportionally
            new_width = target_size
            # Calculate proportional height, round to nearest multiple of 14
            new_height = round(height * (new_width / width) / 14) * 14
        
        # Resize image with new dimensions using high-quality interpolation
        img = img.resize((new_width, new_height), Image.Resampling.BICUBIC)
        
        # Convert PIL image to tensor: [0, 255] -> [0, 1], HWC -> CHW
        img_tensor = TF.to_tensor(img)  # Shape: [C, H, W]
        
        # Apply center crop if height exceeds target size (crop mode only)
        if mode == "crop" and new_height > target_size:
            start_y = (new_height - target_size) // 2
            img_tensor = img_tensor[:, start_y : start_y + target_size, :]
        
        # Apply padding to make square if needed (pad mode only)
        if mode == "pad":
            ...
            if h_padding > 0 or w_padding > 0:
                ...
                # Pad with white background (value=1.0 in normalized space)
                img_tensor = torch.nn.functional.pad(
                    img_tensor, (pad_left, pad_right, pad_top, pad_bottom), 
                    mode="constant", value=1.0
                )
        
        # Move tensor to the model's device for inference
        img_tensor = img_tensor.to(self._device)
        
        logger.debug(f"Preprocessed image: {img_tensor.shape}, original: {original_size}")
